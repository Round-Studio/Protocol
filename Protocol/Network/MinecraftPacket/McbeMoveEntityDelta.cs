...
public class McbeMoveEntityDelta : Packet
{
	...
	protected override void EncodePacket()
	{
		...
		var d = 256f / 360f;
		if ((flags & 0x8) != 0) Write((byte)Math.Round(currentPosition.Pitch * d));

		if ((flags & 0x10) != 0) Write((byte)Math.Round(currentPosition.Yaw * d));

		if ((flags & 0x20) != 0) Write((byte)Math.Round(currentPosition.HeadYaw * d));
	}
